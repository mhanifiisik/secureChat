...
import { PrismaClient, UserStatus, FriendshipStatus } from '@prisma/client'
  ...
  io.on('connection', async (socket: Socket) => {
    ...
    socket.on('send_friend_request', async ({ email }: { email: string }) => {
      try {
        ...
        await emitEventAndUpdateSocketId(socket, 'friend_request_received', { friendship, sender }, receiver.userId)
      } catch (error) {
        socket.emit('error', 'Error sending friend request')
      }
    })

    socket.on('accept_friend_request', async ({ friendshipId }: { friendshipId: string }) => {
      try {
    ...
    socket.on('reject_friend_request', async ({ friendshipId }: { friendshipId: string }) => {
      try {
        const friendship = await prisma.friendship.update({
          where: { friendshipId },
    ...
    socket.on('get_messages', async ({ friendId }: { friendId: string }) => {
      try {
        ...
        console.log('messages seeending')
